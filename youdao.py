...
{% endif %}
        ...
        self.mappingTemplate = {u"单词": jinjaEnv.from_string(u"{% if data.key %}{{data.key}}{% endif %}"),
                                ...
                                u"解释": jinjaEnv.from_string(u"{% for r in data.acceptation %}{% if not loop.first %}<br />{% endif %}{{r}}{% endfor %}"),
    ...
    def query(self, word, forceQuery=False):
        ...
        if not forceQuery:
            ...
            if js:
                ...
                return
        # ------从爱词霸网站上查询------
        conn = httplib.HTTPConnection('fanyi.youdao.com')
        body = urllib.urlencode({'client': 'JTRHelper',
                                   'doctype': 'xml',
                                   'dogVersion': '1.0',
        ...
        ok = False
        try:
            conn.request("POST", "/fsearch", body, headers)
            res = conn.getresponse()
            if res.status == 200:
                # ------提取XML数据------
                xdoc = parseString(res.read())
                self.data = {}
                self.data['ps'] = []
                self.data['ps'].append(xdoc.getElementsByTagName('uk-phonetic-symbol')[0].firstChild.data)
                self.data['ps'].append(xdoc.getElementsByTagName('us-phonetic-symbol')[0].firstChild.data)

                self.data['acceptation'] = [i.firstChild.data for i in xdoc.getElementsByTagName('content')]
...
if len(sys.argv) < 2:
    exit(1)
word = sys.argv[1]
icibaEngine.query(word)
print icibaEngine.html().encode('utf8')